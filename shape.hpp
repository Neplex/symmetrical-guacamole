...
public:
   ...
   */
  virtual bool contains(double const x, double const y) const = 0;
 ...
 *
 * Otherwise a special version has to be made that reads the arguments on the
 * stack.
 */
#define CREATE_INSTANCE(sh)                                                    \
  extern "C" Shape *create_instance(U_Sh_d_Stack &) { return new Shape_##sh(); }

// TODO REMOVE WHEN ALL CREATE_INSTANCE ARE OK
// i.e. all ...._DUMMY should be replaced !
#define CREATE_INSTANCE_DUMMY(sh)                                              \
  extern "C" Shape *create_instance(U_Sh_d_Stack &) { return NULL; }
