 ...
 */
class Bad_Shape_Creation_Exception
 ...
 *
 * \param the c_s U_Sh_d_Stack is here to provide the arguments to build the shape.
 */
...
class Factory_Shape
  ...
  /*!
   * Load all the dynamic library corresponding to the names in factory_shape.config.
   */
  Factory_Shape () ;
...
public:
  ...
  /*!
   * Dynamically load \c shape_ \c name \c .so, get the function name create_instance in it and add the entry \c (name,create_instance)
   ...
   */
  void register_shape ( std :: string const & id , 
			shape_creator s_c ) ;
   ...
   */
  Shape * create_shape ( std :: string const & id ,
			 U_Sh_d_Stack & c_s ) ;
