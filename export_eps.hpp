...
public:
  /** Plot a pixel */
  void plot(unsigned int const x, unsigned int const y);

  /** Open file file_name for writing and send the prologue into it.  */
  Export_Eps(char const *const _file_name, unsigned int const _x_max,
             unsigned int const _y_max, unsigned int const _res = res_defaut);
